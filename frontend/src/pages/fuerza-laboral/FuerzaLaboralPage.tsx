    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 rounded-xl border border-[var(--border)] bg-[var(--bg-surface)]">
        {[
          ...
          { label: 'Centro de Costo', el: <select value={s.department} onChange={(e) => set({ ...s, department: e.target.value })} className="w-full text-sm border border-[var(--border)] rounded-lg px-2 py-1.5 bg-[var(--bg-base)] text-[var(--text-1)] outline-none focus:border-[var(--accent)]"><option value="">General</option>{departments.map((d) => <option key={d.id} value={d.name}>{d.name}</option>)}</select> },
