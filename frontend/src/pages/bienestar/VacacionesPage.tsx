...
import { Plus, Check, X, Trash2, Loader2, Palmtree, AlertCircle } from 'lucide-react'
...
export function VacacionesPage() {
  ...
  const leaves = allLeaves.filter((l) => l.type === 'vacation' && (filterStatus === 'all' || l.status === filterStatus))
  ...
  const createM = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['leaves'] }); toast.success('Solicitud creada'); setShowModal(false); setForm({ employeeId: '', startDate: '', endDate: '', reason: '' }) },
        ...
        <div className="rounded-xl border border-[var(--border)] overflow-hidden bg-[var(--bg-surface)]">
          <table className="w-full">
            <thead className="bg-[var(--bg-hover)]">
              <tr>
                {['Empleado', 'Desde', 'Hasta', 'Días', 'Estado', 'Motivo', ''].map((h) => (
          ...
          <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border)] shadow-2xl w-full max-w-md mx-4 p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={form.employeeId} onChange={(e) => setForm({ ...form, employeeId: e.target.value })}
                  ...
                  {employees.filter(e => e.status === 'active').map((e) => (
                    <option key={e.id} value={e.id}>{e.nombre || `${e.primerNombre} ${e.primerApellido}`}</option>
                  ))}
                </select>
              </div>
              ...
              {form.startDate && form.endDate && (
                <div className="text-xs text-[var(--accent)] font-medium">{calcDays(form.startDate, form.endDate)} día(s) de vacaciones</div>
              )}
              <div>
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <button onClick={() => createM.mutate()} disabled={!form.employeeId || !form.startDate || !form.endDate || createM.isPending}
